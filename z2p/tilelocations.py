...
a coordinate pair and returns a dictionary of location specific
map data 
...
class LocationMap(UserDict):
    ...
    def __init__(self, location_data: List[dict]):
        _location_map = self._form_tile_map(location_data)
        super().__init__()
    ...
    def _form_location_map(self, location_data: List[dict]) -> dict:
        ...
        for location_entry in location_data:
            location_coord = (*location_entry['entrance'])
    ...
    def __set_item__(self, key: Any, value: Any):
        frozen_location_msg = (f'{self} object is immutable '

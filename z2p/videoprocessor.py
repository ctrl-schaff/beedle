...
class VideoProcessor:
    def __init__(
        ...
        self._video_data = self._vax.imshow(self._map_data_RGB, cmap=self.colormap)
    ...
    def animate_explore(self, region_set: list, video_name: str) -> None:
        ...
        full_video = mpy.concatenate_videoclips(self._clip_data, method="compose")
    ...
    def animate_search(self, region_set: list, region_path: list, video_name: str):
        """
        Animate the combination of region exploration and path generation associated with 
        exploring key locations based on map logic
        """
        self.create_explore_video(region_set)
    ...
    def _create_color_map_data(self):
        ...
        self.cmapper = mpl.cm.ScalarMappable(norm=norm_func, cmap=self.colormap)

        map_data_RGBA = self.cmapper.to_rgba(self._map_data_id, None, True, True)
    ...
    def create_path_video(self, path_list: list):
        ...
        for tpath in path_list[0 : self._num_out_path]:
            ...
            all_path_tiles.extend([self._buffer_tile] * self._video_buffer_size)
        self._path_iterator = iter(all_path_tiles)

        for tpath in path_list[0 : self._num_out_path]:
            num_path_frames = tpath.rank[1] + self._video_buffer_size
            path_video = mpy.VideoClip(
                self._make_path_frame, duration=(num_path_frames / self._video_fps)
            )
